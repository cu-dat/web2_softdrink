...
.detail-img img:hover{transform:scale(1.08);}

.title{font-size:32px;font-weight:700;}
.price{color:#0a7c66;font-size:28px;font-weight:bold;margin:20px 0;}

.qty-box{display:flex;gap:10px;align-items:center;margin-bottom:20px;}
.qty-btn{width:40px;height:40px;border:none;background:#ddd;cursor:pointer;}
.qty-input{width:60px;height:40px;text-align:center;}

.add-btn{background:#cbb48b;border:none;padding:12px 30px;font-weight:bold;cursor:pointer;}
.add-btn:hover{background:#bda276;}

.feature-box{background:#fff;padding:20px;border-radius:15px;}
...
.feature-item:hover{background:#f3f3f3;transform:translateX(5px);}
.feature-item img{width:70px;height:70px;object-fit:contain;}
.feature-price{color:#0a7c66;}

.details-box{margin-top:30px;background:#f8f8f8;padding:30px;border-radius:15px;}
...
$img = !empty($product['image'])
    ? "admin/products/uploads/" . $product['image'] // ✅ FIX CHÍNH
...
$img = !empty($item['image'])
    ? "admin/products/uploads/" . $item['image'] // ✅ FIX CHÍNH
    : "assets/images/default.png";
?>

<a href="index.php?page=detail&id=<?= $item['id'] ?>" style="text-decoration:none;color:inherit;">
