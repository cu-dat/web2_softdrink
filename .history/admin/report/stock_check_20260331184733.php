...
SELECT 
    p.name,
    COALESCE(SUM(idt.quantity),0) AS total_import,
    COALESCE(SUM(od.quantity),0) AS total_export
FROM products p
LEFT JOIN import_details idt ON p.id = idt.product_id
LEFT JOIN imports i ON idt.import_id = i.id AND DATE(i.created_at) <= '$date'
LEFT JOIN order_details od ON p.id = od.product_id
LEFT JOIN orders o ON od.order_id = o.id AND DATE(o.created_at) <= '$date'
GROUP BY p.id
";
