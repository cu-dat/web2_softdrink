...
if (!empty($from) && !empty($to)) {
    ...
    $where_condition = "AND DATE(created_at) BETWEEN '$from_esc' AND '$to_esc'";
} elseif (!empty($from)) {
    $from_esc = $conn->real_escape_string($from);
    $where_condition = "AND DATE(created_at) >= '$from_esc'";
} elseif (!empty($to)) {
    $to_esc = $conn->real_escape_string($to);
    $where_condition = "AND DATE(created_at) <= '$to_esc'";
}
...
// Lấy danh sách phiếu XUẤT chi tiết
$exports_sql = "
SELECT 
    o.id AS order_id,
    o.created_at,
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td><?= htmlspecialchars($exp['customer_name'] ?? 'Khách lẻ') ?></td>
