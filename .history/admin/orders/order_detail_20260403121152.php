...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['status'])) {
    ...
    // Xử lý theo từng trạng thái mới
    switch ($newStatus) {
        case 'pending':
            // Chỉ cập nhật trạng thái, không động đến kho
            $updateStmt = $conn->prepare("UPDATE orders SET status = 'pending' WHERE id = ?");
            $updateStmt->bind_param("i", $id);
            $updateStmt->execute();
            setFlashMessage('success', 'Đã chuyển trạng thái về Chờ xử lý.');
            header("Location: order_detail.php?id=$id");
            exit();
            break;
        ...
        case 'confirmed':
        case 'completed':
            // Chuyển hướng sang order_complete.php kèm theo trạng thái đích
            header("Location: order_complete.php?id=$id&new_status=$newStatus");
            exit();
            break;

        case 'cancelled':
            // Chuyển hướng sang order_cancel.php
            header("Location: order_cancel.php?id=$id");
            exit();
            break;
    }
}
                    ...
                    <table class="table table-bordered text-center align-middle">
                        ...
                        <tbody>
                            ...
                            <?php if ($details->num_rows == 0): ?>
                                <tr>
                                    <td colspan="4" class="text-muted">Không có sản phẩm nào trong đơn hàng.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
