...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['status'])) {
    ...
    // Nếu trạng thái mới giống cũ thì không làm gì
    ...
    try {
        // Lấy danh sách sản phẩm trong đơn (cần cho cả trừ và cộng)
        $items = $conn->query("SELECT product_id, quantity FROM order_details WHERE order_id = $id");
        if (!$items) {
            throw new Exception("Không thể lấy chi tiết đơn hàng!");
        }

        // 1. Xử lý trừ kho khi chuyển từ pending sang confirmed hoặc completed
        if ($currentStatus === 'pending' && in_array($newStatus, ['confirmed', 'completed'])) {
            ...
            while ($item = $items->fetch_assoc()) {
                ...
                if ($currentStock < $qty) {
                    $insufficient[] = "ID $pid (cần $qty, còn $currentStock)";
                }
            }
            if (!empty($insufficient)) {
                throw new Exception("Không đủ tồn kho: " . implode(', ', $insufficient));
            }
        ...
        // 2. Xử lý hoàn kho khi hủy đơn (cancelled) mà đơn đã được xác nhận hoặc hoàn thành
        if ($newStatus === 'cancelled' && in_array($currentStatus, ['confirmed', 'completed'])) {
            $items->data_seek(0);
            while ($item = $items->fetch_assoc()) {
                $pid = $item['product_id'];
                $qty = (int)$item['quantity'];
        ...
        // Chuyển hướng về trang chi tiết để thấy trạng thái mới
        header("Location: order_detail.php?id=$id");
        exit();
    } catch (Exception $e) {
        $conn->rollback();
...
<div class="container mt-4">
    <div class="card shadow">
        ...
        <div class="card-body">
            ...
            <?php else: ?>
                <div class="alert alert-warning no-remove">⚠️ Đơn hàng đã hoàn tất hoặc bị huỷ, không thể chỉnh sửa trạng thái.</div>
            <?php endif; ?>
