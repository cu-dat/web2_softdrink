...
$order_details = $conn->query("SELECT product_id, quantity FROM order_details WHERE order_id = $order_id");
while ($item = $order_details->fetch_assoc()) {
    updateInventory($conn, $item['product_id'], -$item['quantity']); // trừ tồn
}
// ===== 5. UPDATE STATUS =====
$conn->query("
    UPDATE orders 
